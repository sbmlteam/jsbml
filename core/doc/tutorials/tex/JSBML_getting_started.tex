...
ontology-related classes \citep{Holland2008}.\index{Ontology}
...
The command above downloads the entire \texttt{trunk} that is structured into
the sub-directories \texttt{core}, \texttt{extensions}, \texttt{modules}. Each
of these sub-folders can separately be checked out. The \texttt{core} folder
contains all classes, libraries, and other resources that are needed to work
with the SBML core, i.e., without extensions. For working with extension
packages, the \texttt{extensions} folder contains a separate repository for each
currently implemented extension package. You can either do an individual
checkout each such package, or obtain the complete \texttt{extensions}
repository. Finally, the \texttt{modules} provide additional functionality for
JSBML and are described in more detail in the next paragraph.
...
JSBML provides today, five additional modules. Binary versions of the modules
can be found at the download site of JSBML. In order to obtain the most recent
version of the modules, please type the following Subversion\footref{fn:svn}
commands on your command line.

\begin{lstlisting}[language=bash,numbers=none,captionpos=t,
title={The Android-compatibility module provides interfaces needed to run
your JSBML-based program on Android OS.}]
svn co \
"https://jsbml.svn.sourceforge.net/svnroot/jsbml/trunk/modules/android" \
Android
\end{lstlisting}
...
svn co \
"https://jsbml.svn.sourceforge.net/svnroot/jsbml/trunk/modules/celldesigner" \
CellDesigner
\end{lstlisting}

\begin{lstlisting}[language=bash,numbers=none,captionpos=t,
title={The compare module can be used to draw comparisons between libSBML and
JSBML.}]
svn co \
"https://jsbml.svn.sourceforge.net/svnroot/jsbml/trunk/modules/compare" \
Compare
\end{lstlisting}

\begin{lstlisting}[language=bash,numbers=none,captionpos=t,
title={The libSBMLcompat module allows a slow and easy switch between libSBML
and JSBML by providing wrapper classes with the identical API as in libSBML.}]
svn co \
"https://jsbml.svn.sourceforge.net/svnroot/jsbml/trunk/modules/libSBMLcompat" \
libSBMLcompat
\end{lstlisting}
...
svn co \
"https://jsbml.svn.sourceforge.net/svnroot/jsbml/trunk/modules/libSBMLio/" \
libSBMLio
\end{lstlisting}

Note that currently only the celldesigner and the libSBMLio module are
extensively tested.
