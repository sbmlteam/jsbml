...
\TreeNode, all derived elements of \texttt{SBase} also implement these
types. In this way, derivatives of \texttt{SBase} can be used wherever an
instance of \TreeNode is requested. Furthermore, SBML elements that do
...
\begin{figure}[p]
 ...
 of \ASTNode, which can be converted to or read from \texttt{C}-like
 formula \texttt{String}s. Internally, \texttt{AbstractMathContainer}s only
 deal with instances of \ASTNode. It should be noted that these
...
\begin{sidewaysfigure}[htbp]
 ...
 JSBML, which is done in form of \ASTNode{} objects. These can be
...
\Serializable, % and \TreeNode. As all elements defined in JSBML
\index{cloning}%
...
\TreeNode{} is actually defined in Java's \texttt{swing}
\index{graphical user interface!swing@\texttt{swing}}%
package.
\TreeNode{} is a type that is independent of any graphical information. It
...
../posters/2010_ICSB_and_COMBINE/JSBMLvisualizerTransparent}
  \caption[Tree representation of an SBML file]{A tree representation of the
    ...
    instances of \texttt{SBase} implement the interface \TreeNode.}
  \label{fig:Visualization}
  \end{SCfigure}
}{%
}%
The \ASTNode{} class in JSBML is also
...
\TreeNode. In addition, it also provides methods to notify dedicated
...
Some types derived from \SBase{} contain an identifier, a so-called
\texttt{id}. JSBML gathers all these elements under the common interface
\NamedSBase. The class \AbstractNamedSBase, which extends
...
\NamedSBase{} must be unique if these are defined. The Boolean method
\texttt{isIdMandatory()} in \NamedSBase{} indicates if an identifier must
...
\texttt{SBase} in form of an abstract syntax tree, i.e., \ASTNode.
...
extends \AbstractNamedSBase{} and
...
Both libraries define a class \ASTNode{}
...
language-like \texttt{String}s\index{String@\texttt{String}!formula}, or from a
MathML\index{MathML} representation. The JSBML
\ASTNode{} provides various methods to transform these trees to other
formats, for instance, \LaTeX{}\index{LaTeX@\LaTeX} \texttt{String}s. In JSBML,
several static methods allow easy creation of new syntax trees, for instance,
the following code
\begin{lstlisting}[language=Java,numbers=none]
ASTNode myNode = ASTNode.plus(myLeftAstNode, myRightASTNode);
\end{lstlisting}
creates a new instance of \ASTNode{} which represents the sum of the two
other {\ASTNode}s. In this way, even complex trees can be easily
manipulated.
...
common interface \CallableSBase,
...
\end{lstlisting}
The set method allows users to change the type of an \ASTNode{} to
...
\ASTNode{} is something different from \texttt{ASTNode.Type.NAME}, an
\index{ASTNode@\texttt{ASTNode}!ASTNode.Type@\texttt{ASTNode.Type}}%
...
interface, for instance, \ASTNode{} objects can be directly translated to
...
\index{species!\texttt{Species}}%
to a \Model, libSBML\index{cloning} will
...
SBML \index{SBML!hierarchical structure} define a tree, in which each element
...
\begin{enumerate}
  ...
  \item The list of
  \texttt{TreeNodeChangeListener}s\index{event!\texttt{TreeNodeChangeListener}}
...
\index{parameter!\texttt{constant}}%
 ...
 instance of \AbstractNamedSBase.
 ...
 \item In case that spatial dimension units of a \Species{} are defined whose
 surrounding \Compartment{} has zero dimensions or that has only substance
 units, JSBML also throws an exception.
\end{itemize}
...
object \History{} in JSBML, whereas it is still called \ModelHistory{} in
libSBML. Hence, all instances of \SBase{} in JSBML contain methods to access
and manipulate its \History. Furthermore, you will not find the classes
\ModelCreator{} and \texttt{ModelCreatorList} because JSBML gathers its
\texttt{Creator} objects in a generic \texttt{List<Creator>} in the \History.
...
with the prefix \ASTtype{}
have a corresponding field in the \ASTNode{} class itself. There you can
...
\end{lstlisting}
In a similar way, the \ASTNode{} class
...
useful to either check if the \Model{}
contains this \texttt{UnitDefinition}
\index{unit!UnitDefinition@\texttt{UnitDefinition}}%
or to add it to the \Model.
...
\Model{} does not contain separate \texttt{UnitDefinition}s for both
entries.
...
\begin{description}
 ...
 that the given \TreeNode{} has just
 ...
 \item[\texttt{nodeRemoved(TreeNode node)}] The \TreeNode{} instance
 ...
 \TreeNode{} that has been changed, its
...
Since in JSBML all major data objects implement the interface \TreeNode,
...
standard \TreeNode{} interface by adding methods that maintain a list of
...
which creates then a JSBML \Model{} data structure.
