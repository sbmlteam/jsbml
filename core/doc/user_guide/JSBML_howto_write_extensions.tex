...
public class Foo extends AbstractNamedSBase {
  ...
  public Map<String, String> writeXMLAttributes() {
    ...
    if (isSetBar()) {
      ...
      // Note that in case of double values, the user's locale needs to be taken into account in
      // order to prevent the Writer from numbers in the wrong format. Even in the case of Integer it
      // can be important, because in some languages very strange number symbols are used (e.g., Thai
      // or Chinese) and hence, the UTF-8 encoding of XML in SBML will lead to SBML files that cannot
      // be parsed again. SBML only accepts English doubles. Since bar represents an integer, less
