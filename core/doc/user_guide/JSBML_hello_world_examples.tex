...
This program illustrate also the preferred way to create JSBML object instances. 
The only constructor you should need to use is the constructor of the \SBMLDocument, specifying
the SBML level and version you want to use. Then each JSBML class should have some 
\code{createXXX} methods where 'XXX' is the children class name. 
For example, \code{model.createSpecies(String)}, \code{model.createReaction(String)}
or \code{reaction.createReactant()}. These methods will guaranty that you are creating a proper
...
JSBML \index{CellDesigner} (See \vref{sec:jsbml-modules-details} to learn more
on JSBML Modules in general). Some more complex examples are available from the JSBML 
SourceForge repository, see \vref{sec:jsbml-repo-examples} to know how to obtain them.
