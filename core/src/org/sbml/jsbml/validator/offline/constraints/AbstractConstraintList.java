...
public abstract class AbstractConstraintList {
    ...
    {
	String className = "L" + level + "V" + version + StringTools.firstLetterUpperCase(pkg.toString()) + "ConstraintList";
	
	Class<?> listClass = getFromCache(className);
	
	if (listClass == null)
	{
	    String fullName = AbstractConstraintList.class.getPackage().toString() + "." + className;
    ...
    public static List<Integer> getIdsForClass(Class<?> clazz, CheckCategory category, SBMLPackage[] packages, int level, int version)
    {
	...
	for (SBMLPackage pkg: packages)
	{
	    ...
	    {
		String methodName = "add" + StringTools.firstLetterUpperCase(category.toString()) + clazz.getSimpleName() + "Ids";
		    try {
