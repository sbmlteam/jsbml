...
public class SyntaxChecker {
   ...
   *
   * @param idCandidate, level, version
   * @return {@code true} if the given argument is a valid meta identifier
   *         {@link String}, {@code false} otherwise.
   */
  ...
  //TODO - back to its original state 
  /**
  ...
  private void initMetaIdPatterns() {
    metaIdPattern = null;
    String underscore = "_";
    String letter = "[\u0041-\u005A]|[\u0061-\u007A]|[\u00C0-\u00D6]|[\u00D8-\u00F6]|[\u00F8-\u00FF]|[\u0100-\u0131]|[\u0134-\u013E]|[\u0141-\u0148]" 
        + "|[\u014A-\u017E]|[\u0180-\u01C3]|[\u01CD-\u01F0]|[\u01F4-\u01F5]|[\u01FA-\u0217]|[\u0250-\u02A8]|[\u02BB-\u02C1]"  
        + "|\u0386|[\u0388-\u038A]|\u038C|[\u038E-\u03A1]|[\u03A3-\u03CE]|[\u03D0-\u03D6]|\u03DA|\u03DC|\u03DE|\u03E0|[\u03E2-\u03F3]"  
        ...
        + "|[\u0AB2-\u0AB3]|[\u0AB5-\u0AB9]|\u0ABD|\u0AE0|[\u0B05-\u0B0C]|[\u0B0F-\u0B10]|[\u0B13-\u0B28]|[\u0B2A-\u0B30]|[\u0B32-\u0B33]"  
        ...
        + "|[\u0E32-\u0E33]|[\u0E40-\u0E45]|[\u0E81-\u0E82]|\u0E84|[\u0E87-\u0E88]|\u0E8A|\u0E8D|[\u0E94-\u0E97]|[\u0E99-\u0E9F]" 
        + "|[\u0EA1-\u0EA3]|\u0EA5|\u0EA7|[\u0EAA-\u0EAB]|[\u0EAD-\u0EAE]|\u0EB0|[\u0EB2-\u0EB3]|\u0EBD|[\u0EC0-\u0EC4]|[\u0F40-\u0F47]" 
        + "|[\u0F49-\u0F69]|[\u10A0-\u10C5]|[\u10D0-\u10F6]|\u1100|[\u1102-\u1103]|[\u1105-\u1107]|\u1109|[\u110B-\u110C]|[\u110E-\u1112]" 
        + "|\u113C|\u113E|\u1140|\u114C|\u114E|\u1150|[\u1154-\u1155]|\u1159|[\u115F-\u1161]|\u1163|\u1165|\u1167|\u1169|[\u116D-\u116E]" 
        + "|[\u1172-\u1173]|\u1175|\u119E|\u11A8|\u11AB|[\u11AE-\u11AF]|[\u11B7-\u11B8]|\u11BA|[\u11BC-\u11C2]|\u11EB|\u11F0|\u11F9" 
        + "|[\u1E00-\u1E9B]|[\u1EA0-\u1EF9]|[\u1F00-\u1F15]|[\u1F18-\u1F1D]|[\u1F20-\u1F45]|[\u1F48-\u1F4D]|[\u1F50-\u1F57]|\u1F59|\u1F5B" 
        + "|\u1F5D|[\u1F5F-\u1F7D]|[\u1F80-\u1FB4]|[\u1FB6-\u1FBC]|\u1FBE|[\u1FC2-\u1FC4]|[\u1FC6-\u1FCC]|[\u1FD0-\u1FD3]|[\u1FD6-\u1FDB]" 
        + "|[\u1FE0-\u1FEC]|[\u1FF2-\u1FF4]|[\u1FF6-\u1FFC]|\u2126|[\u212A-\u212B]|\u212E|[\u2180-\u2182]|[\u3041-\u3094]|[\u30A1-\u30FA]" 
        + "|[\u3105-\u312C]|[\uAC00-\uD7A3]" //Basechar
        + "[\u4E00-\u9FA5]|\u3007|[\u3021-\u3029]"; //Ideographic
    String digit = "[\u0030-\u0039]|[\u0660-\u0669]|[\u06F0-\u06F9]|[\u0966-\u096F]|[\u09E6-\u09EF]"
        + "|[\u0A66-\u0A6F]|[\u0AE6-\u0AEF]|[\u0B66-\u0B6F]|[\u0BE7-\u0BEF]|[\u0C66-\u0C6F]"
        + "|[\u0CE6-\u0CEF]|[\u0D66-\u0D6F]|[\u0E50-\u0E59]|[\u0ED0-\u0ED9]|[\u0F20-\u0F29]";
  ...
  //overloaded method with level and version to be clear which patterns should be initialized 
  //TODO - vielleicht gibts eleganteren Weg als switch, vielleicht doch mit if? 
  private String initMetaIdPatterns(int level, int version) {
    ...
    case 1: {
      String sName;
      if(version == 1) {
        //see under specification for SBML L1V1: 3.3 Type SName 
        sName = underscore + "*" + "[" + letter + "]" + "[" + letter +  underscore + digit + "]*";
        metaIdPattern = Pattern.compile(sName);
        System.out.println("level 1, version 1: attempt to load patterns: " + sName);
        return sName;     
      } else if(version == 2) { 
        //see under specification for SBML L1V2: 3.3 Type SName 
        sName = "[" + letter + underscore + "]" + "[" + letter + digit + underscore + "]*";
        metaIdPattern = Pattern.compile(sName);
        System.out.println("level 1, version 2: attempt to load patterns: " + sName);
        return sName; 
      }
      System.out.println("no valid version, could not load pattern correctly");
      break;
    } 

    case 2: {  
      if(version == 1) {  //TODO - gleich wie L1V2 also zusammenführen
        //Id has Type Sid, see Specification of SBML L2V1 under 3.4
        String nameChar = letter + digit + underscore; 
        String name = "[" + letter + underscore + "]" + "[" + nameChar + "]*";
        metaIdPattern = Pattern.compile(name);
        System.out.println("level 2, version 1: attempt to load patterns: " + name);
        return name; 

      } else if(version == 2) {     
        //Id is TypeID, see Specification of SBML L2V2R1 under 3.1.6
        StringBuilder combiningChar = StringTools.concatStringBuilder("[\u0300-\u0345][\u0360-\u0361][\u0483-\u0486][\u0591-\u05A1][\u05A3-\u05B9][\u05BB-\u05BD]"
          , "\u05BF[\u05C1-\u05C2]\u05C4[\u064B-\u0652]\u0670[\u06D6-\u06DC][\u06DD-\u06DF][\u06E0-\u06E4][\u06E7-\u06E8]"
          , "[\u06EA-\u06ED][\u0901-\u0903]\u093C[\u093E-\u094C]\u094D[\u0951-\u0954][\u0962-\u0963][\u0981-\u0983]"
        ...
        String extender = "\u00B7\u02D0\u02D1\u0387\u0640\u0E46\u0EC6\u3005[\u3031-\u3035][\u309D-\u309E][\u30FC-\u30FE]";
        String ncNameChar = letter + digit  + dot  + dash + underscore + combiningChar + extender;
        String id = "[" + letter + underscore + "]" + "[" + ncNameChar + "]*"; 
        System.out.println("level 2, version 2: attempt to load patterns: " + id);
        metaIdPattern = Pattern.compile(id);
        return id;     

      } else if(version == 3 || version == 4 || version == 5) {        //TODO - prüfe ob das wahr ist 
        digit = "[\u0030-\u0039][\u0660-\u0669][\u06F0-\u06F9][\u0966-\u096F][\u09E6-\u09EF]"
            + "[\u0A66-\u0A6F][\u0AE6-\u0AEF][\u0B66-\u0B6F][\u0BE7-\u0BEF][\u0C66-\u0C6F]"
            + "[\u0CE6-\u0CEF][\u0D66-\u0D6F][\u0E50-\u0E59][\u0ED0-\u0ED9][\u0F20-\u0F29]";
       StringBuilder letterBuilder = StringTools.concatStringBuilder("[\u0041-\u005A][\u0061-\u007A][\u00C0-\u00D6][\u00D8-\u00F6][\u00F8-\u00FF][\u0100-\u0131][\u0134-\u013E][\u0141-\u0148]"
          , "[\u014A-\u017E][\u0180-\u01C3][\u01CD-\u01F0][\u01F4-\u01F5][\u01FA-\u0217][\u0250-\u02A8][\u02BB-\u02C1]"  
          ...
          , "[\u3105-\u312C][\uAC00-\uD7A3]" 
          , "[\u4E00-\u9FA5]\u3007[\u3021-\u3029]");
        StringBuilder combiningCharBuilder = StringTools.concatStringBuilder("[\u0300-\u0345][\u0360-\u0361][\u0483-\u0486][\u0591-\u05A1][\u05A3-\u05B9][\u05BB-\u05BD]"
          , "\u05BF[\u05C1-\u05C2]\u05C4[\u064B-\u0652]\u0670[\u06D6-\u06DC][\u06DD-\u06DF][\u06E0-\u06E4][\u06E7-\u06E8]"
          , "[\u06EA-\u06ED][\u0901-\u0903]\u093C[\u093E-\u094C]\u094D[\u0951-\u0954][\u0962-\u0963][\u0981-\u0983]"
          , "\u09BC\u09BE\u09BF[\u09C0-\u09C4][\u09C7-\u09C8][\u09CB-\u09CD]\u09D7[\u09E2-\u09E3]\u0A02\u0A3C\u0A3E"
          , "\u0A3F[\u0A40-\u0A42][\u0A47-\u0A48][\u0A4B-\u0A4D][\u0A70-\u0A71][\u0A81-\u0A83]\u0ABC[\u0ABE-\u0AC5]"
          ...
          , "[\u0F71-\u0F84][\u0F86-\u0F8B][\u0F90-\u0F95]\u0F97[\u0F99-\u0FAD][\u0FB1-\u0FB7]\u0FB9[\u20D0-\u20DC]\u20E1[\u302A-\u302F]\u3099\u309A");
        String extender = "\u00B7\u02D0\u02D1\u0387\u0640\u0E46\u0EC6\u3005[\u3031-\u3035][\u309D-\u309E][\u30FC-\u30FE]";
        String ncNameChar = "[" + letterBuilder + digit + dot + dash + underscore + colon + combiningCharBuilder + extender + "]";
        String id = "[" + letterBuilder + underscore + colon + "]" + "[" + ncNameChar + "]*";    
        metaIdPattern = Pattern.compile(id);
        return id;  
    ...
    case 3: {      
      if(version == 1 || version == 2) {    //TODO - Ist das nicht gleich wie L2 V3,V4,V5???
        digit = "[\u0030-\u0039][\u0660-\u0669][\u06F0-\u06F9][\u0966-\u096F][\u09E6-\u09EF]"
            + "[\u0A66-\u0A6F][\u0AE6-\u0AEF][\u0B66-\u0B6F][\u0BE7-\u0BEF][\u0C66-\u0C6F]"
            + "[\u0CE6-\u0CEF][\u0D66-\u0D6F][\u0E50-\u0E59][\u0ED0-\u0ED9][\u0F20-\u0F29]";
       StringBuilder letterBuilder = StringTools.concatStringBuilder("[\u0041-\u005A][\u0061-\u007A][\u00C0-\u00D6][\u00D8-\u00F6][\u00F8-\u00FF][\u0100-\u0131][\u0134-\u013E][\u0141-\u0148]"
      ...
      System.out.println("no valid or unknown version, could not load pattern correctly");
      break;
      //when new version comes out method should be updated 
    }
    }
    return null;
  }
