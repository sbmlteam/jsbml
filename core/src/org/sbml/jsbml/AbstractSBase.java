...
public abstract class AbstractSBase extends AbstractTreeNode implements SBase {
  ...
  protected boolean checkAttribute(String attributeName) {
    ...
    if (!isReadingInProgress()) {
      AbstractSBase.attributeValidator.setLevelAndVersion(getLevel(), getVersion());

      logger.warn("Invalid value for attribute " + attributeName + " on SBase with id = " + metaId +"!");
  ...
  public SBasePlugin getPlugin(String nameOrUri) {
    SBasePlugin plugin = getExtension(nameOrUri);
    if (plugin != null) {
      return plugin;
    }
