...
public abstract class AbstractSBase extends AbstractTreeNode implements SBase {
  ...
  /**
   * Stores the value of the invalid attribute in the user objects HashMap. The
   * key which is used will be generated by
   * <code> "InvalidAttribute" + attributeName</code>. To allow multiple
   * invalid values, the value will be stored in a {@link LinkedList}.
   * This method only works while reading is in progress.
   * 
   * @param attributeName
   * @param value
   */
  public void addInvalidAttribute(String attributeName, Object value) {
    if (this.isReadingInProgress()) {
      
      Queue<Object> invalidAttribute = getInvalidAttribute(attributeName);

      if (invalidAttribute == null) {
        String key = generateKeyForInvalidAttribute(attributeName);
      
        invalidAttribute = new LinkedList<Object>();
        this.putUserObject(key, invalidAttribute);
      }

      invalidAttribute.offer(value);
    }
  }
  ...
  private String generateKeyForInvalidAttribute(String attributeName) {
    return "InvalidAttribute" + attributeName;
  }
  ...
  /**
   * Returns a {@link Queue} which contains every value which was tried to set
   * for the attribute, but was rejected.
   * 
   * If no invalid value was set, this method returns <code>null</code>.
   * 
   * @param attributeName
   * @return a {@link Queue} with the invalid values or <code>null</code>.
   */
  @SuppressWarnings("unchecked")
  public Queue<Object> getInvalidAttribute(String attributeName) {
    
    String key = generateKeyForInvalidAttribute(attributeName);
    Object userObject = this.getUserObject(key);
  

    if (userObject != null && userObject instanceof Queue<?>) {
      return (Queue<Object>) userObject;
    }
