...
public class ASTFactoryTest {
   ...
   */
  @Test
  public final void testReduceToBinary() {
    ASTCnIntegerNode[] integerNodeList = new ASTCnIntegerNode[10];
    for (int i = 0; i < 10; i++) {
      integerNodeList[i] = new ASTCnIntegerNode(i);
    }
    ASTFactory.sum(integerNodeList);
    ASTNode2[] parentNodeList = new ASTNode2[10];
    for (int j = 0; j < 10; j++) {
      parentNodeList[j] = (ASTNode2)integerNodeList[j].getParent();
    }
    ArrayList<ASTNode2> filteredParentNodes = new ArrayList<ASTNode2>();
    for (ASTNode2 parentNode : parentNodeList) {
      if (!filteredParentNodes.contains(parentNode)) {
        filteredParentNodes.add(parentNode);
      }
    }
    assertTrue(filteredParentNodes.size() == 9);
  }
   ...
   */
  @Test
  public final void testProductNoArgument() {
    exception.expect(NullPointerException.class);
    ASTNode2[] list = null;
