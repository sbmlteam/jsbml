 ...
 * string “Cell Death” as value. Additionally, as one may need to convert a
 * dynamic Event between different representations (e.g., Cellular PottsModel
 * vs. Center-based off-latticeModel), there is a need to provide a standard,
 * framework-independent, way of associating Event components with given
 * cellular processes. A solution inspired by SBML Level 3 Version 1 Core is to
 * associate model components with terms from carefully curated controlled
	...
	public static class Term implements Cloneable, Comparable<Term>,
		...
		public Set<Term> getChildren() {
			...
			for (Triple triple : childrenRelationShip) {
				children.add((Term) triple.getSubject());
			}
		...
		public Set<Term> getParentTerms() {
			...
			for (Triple triple : parentRelationShip) {
				parents.add((Term) triple.getObject());
			}
