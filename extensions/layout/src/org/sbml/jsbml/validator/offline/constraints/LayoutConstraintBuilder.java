...
public class LayoutConstraintBuilder extends AbstractConstraintBuilder
  implements SpecialLayoutErrorCodes {
    ...
    case ID_VALIDATE_LAYOUT_TREE:
      ValidationFunction<Layout> f5 = new ValidationFunction<Layout>() {
        ...
        public boolean check(ValidationContext ctx, Layout l) {
          ...
          {
            AnyConstraint<CompartmentGlyph> c = factory.getConstraintsForClass(
              CompartmentGlyph.class, ctx.getCheckCategories(),
              ctx.getPackages(), ctx.getLevel(), ctx.getVersion());
          ...
          {
            AnyConstraint<TextGlyph> c = factory.getConstraintsForClass(
              TextGlyph.class, ctx.getCheckCategories(), ctx.getPackages(),
              ctx.getLevel(), ctx.getVersion());
          ...
          {
            AnyConstraint<TreeNodeChangeListener> c =
              factory.getConstraintsForClass(TreeNodeChangeListener.class,
                ctx.getCheckCategories(), ctx.getPackages(), ctx.getLevel(),
                ctx.getVersion());
