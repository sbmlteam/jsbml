...
public class FBCModelPlugin extends AbstractSBasePlugin {
  ...
  private ListOfObjectives listOfObjectives = new ListOfObjectives();
	...
	/**
	 * Gets a {@link FluxBound} from the listOfFluxBounds at the given index.
	 * 
	 * @param i the index of the {@link FluxBound}
	 * @return
	 * @throws IndexOutOfBoundsException if the listOf is not set or
	 * if the index is out of bound (index < 0 || index > list.size)
	 */
	public FluxBound getFluxBound(int i) {
		if (isSetListOfFluxBounds() && (i >= 0) && (i < listOfFluxBounds.size())) {
