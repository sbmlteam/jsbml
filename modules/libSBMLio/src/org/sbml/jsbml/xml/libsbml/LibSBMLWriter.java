  ...
  private static final Logger logger = Logger.getLogger(LibSBMLWriter.class);
	...
	private boolean equal(ASTNode math, org.sbml.libsbml.ASTNode libMath) {
		...
		case RATIONAL:
			...
			break;
		case NAME_AVOGADRO:
      equal &= libMath.getType() == libsbmlConstants.AST_NAME_AVOGADRO;
		  break;
		case NAME_TIME:
	  ...
	  //TODO: Not yet supported by libSBML!
    // equal &= libMath.getType() == libsbmlConstants.AST_FUNCTION_SELECTOR;
		// break;
		case FUNCTION_SIN:
	...
	private boolean equal(Unit u, org.sbml.libsbml.Unit unit) {
		...
		equal &= u.getExponent() == unit.getExponentAsDouble();
	...
	public int getErrorCount(Object sbase) {
		if (!(sbase instanceof org.sbml.libsbml.SBase)) {
			throw new IllegalArgumentException(MessageFormat.format(error,
			  "sbase", org.sbml.libsbml.SBase.class.getName()));
		}
		org.sbml.libsbml.SBMLDocument doc =
		    ((org.sbml.libsbml.SBase) sbase).getSBMLDocument();
	...
	public List<SBMLException> getWriteWarnings(Object sbase) {
		if (!(sbase instanceof org.sbml.libsbml.SBase)) {
			throw new IllegalArgumentException(MessageFormat.format(error,
					"sbmlDocument", org.sbml.libsbml.SBase.class.getName()));
		}
		org.sbml.libsbml.SBMLDocument doc =
		    ((org.sbml.libsbml.SBase) sbase).getSBMLDocument();
		doc.checkConsistency();
	...
	public void removeUnneccessaryElements(Model model, Object orig) {
		if (!(orig instanceof org.sbml.libsbml.Model)) {
			throw new IllegalArgumentException("only instances of "
			    + org.sbml.libsbml.Model.class.getName() + " can be considered.");
		}
		...
		for (i = mo.getNumRules() - 1; i >= 0; i--) {
			...
			boolean contains = false;
			for (int j = 0; j < model.getRuleCount() && !contains; j++) {
				...
				if (((c instanceof org.sbml.libsbml.RateRule)
						...
						|| (c instanceof org.sbml.libsbml.AssignmentRule
								...
								.getVariable().equals(
										((org.sbml.libsbml.AssignmentRule) c).getVariable()))
		...
		for (i = mo.getNumConstraints() - 1; i >= 0; i--) {
			...
			boolean contains = false;
			for (int j = 0; j < model.getConstraintCount() && !contains; j++) {
	...
	/* (non-Javadoc)
	 * @see org.sbml.squeezer.io.AbstractSBMLWriter#saveChanges(org.sbml.Model, java.lang.Object)
	 */
	public boolean saveChanges(Model model, Object orig) throws SBMLException {
		...
		for (i = 0; i < model.getInitialAssignmentCount(); i++) {
			...
			for (long j = 0; j < modelOrig.getNumInitialAssignments()
					&& (contains < 0); j++) {
				org.sbml.libsbml.InitialAssignment libIA = modelOrig.getInitialAssignment(j);
			...
			if (contains < 0) {
				...
				saveMathContainerProperties(ia, modelOrig.getInitialAssignment(contains));
			}
			fireIOEvent(ia);
		}
	...
	/* (non-Javadoc)
	 * @see org.sbml.jsbml.io.AbstractSBMLWriter#saveChanges(org.sbml.jsbml.Reaction, java.lang.Object)
	 */
		...
		if (reaction.isSetKineticLaw() && reaction.getKineticLaw().isSetMath()) {
			...
			for (FunctionDefinition fd : m.getListOfFunctionDefinitions()) {
				if (math.refersTo(fd.getId())) {
					if (pluMo.getFunctionDefinition(fd.getId()) == null) {
						pluMo.addFunctionDefinition(writeFunctionDefinition(fd));
					} else {
						saveMathContainerProperties(fd, pluMo.getFunctionDefinition(fd.getId()));
					}
				}
			}
			for (Compartment c : m.getListOfCompartments()) {
				if (math.refersTo(c.getId())) {
					if (c.isSetUnits()
							&& !Unit.isUnitKind(c.getUnits(), c.getLevel(), c.getVersion())) {
						pluMo.addUnitDefinition(writeUnitDefinition(c.getUnitsInstance()));
					}
					if (c.isSetCompartmentType()
							&& pluMo.getCompartmentType(c.getCompartmentType()) == null) {
						pluMo.addCompartmentType(writeCompartmentType(c.getCompartmentTypeInstance()));
					}
					if (pluMo.getCompartment(c.getId()) == null) {
						pluMo.addCompartment(writeCompartment(c));
					} else {
						saveCompartmentProperties(c, pluMo.getCompartment(c.getId()));
					}
				}
			}
				...
				if (math.refersTo(p.getId())) {
					if (p.isSetUnits()
						...
						if (pluMo.getUnitDefinition(p.getUnits()) == null) {
							pluMo.addUnitDefinition(writeUnitDefinition(p.getUnitsInstance()));
						} else {
					...
					if (pluMo.getParameter(p.getId()) == null) {
						...
						saveParameterProperties(p, pluMo.getParameter(p.getId()));
					}
				}
			}
		}
	...
	private void saveChanges(Species species, org.sbml.libsbml.Model pluMo) {
		...
		if (species.isSetSubstanceUnits()
			...
			if (pluMo.getUnitDefinition(species.getSubstanceUnits()) == null) {
				pluMo.addUnitDefinition(writeUnitDefinition(species.getSubstanceUnitsInstance()));
			} else {
				saveUnitDefinitionProperties(species.getSubstanceUnitsInstance(), pluMo
	...
	/* (non-Javadoc)
	 * @see org.sbml.jlibsbml.SBMLWriter#saveCompartmentProperties(org.sbml.jlibsbml.Compartment, java.lang.Object)
	 */
	...
	private void saveEventProperties(Event ev, Object event)
		...
		for (EventAssignment ea : ev.getListOfEventAssignments()) {
			...
			for (long i = 0; i < e.getNumEventAssignments() && contains < 0; i++) {
				org.sbml.libsbml.EventAssignment libEA = e.getEventAssignment(i);
		...
		for (long i = e.getNumEventAssignments() - 1; i >= 0; i--) {
			...
			boolean contains = false;
			for (int j = 0; j < ev.getEventAssignmentCount() && !contains; j++) {
	...
	private void saveKineticLawProperties(KineticLaw kl, Object kineticLaw)
		...
		for (LocalParameter p : kl.getListOfLocalParameters()) {
			org.sbml.libsbml.Parameter libParam = libKinLaw.getParameter(p.getId());
			if (p.isSetUnits()
					&& !Unit.isUnitKind(p.getUnits(), p.getLevel(), p.getVersion())
		...
		for (long i = libKinLaw.getNumParameters() - 1; i >= 0; i--) {
			...
			if (kl.getLocalParameter(p.getId()) == null) {
	...
	/* (non-Javadoc)
	 * @see org.sbml.jlibsbml.SBMLWriter#saveMathContainerProperties(org.sbml.jlibsbml.MathContainer, java.lang.Object)
	 */
	...
	/* (non-Javadoc)
	 * @seeorg.sbml.SBMLWriter#saveModifierSpeciesReferenceProperties(org.sbml.ModifierSpeciesReference, java.lang.Object)
	 */
	...
	/* (non-Javadoc)
	 * @see org.sbml.SBMLWriter#saveNamedSBaseProperties(org.sbml.NamedSBase, java.lang.Object)
	 */
	private void saveNamedSBaseProperties(NamedSBase nsb, Object sb) {
		if (!(sb instanceof org.sbml.libsbml.SBase)) {
			throw new IllegalArgumentException(MessageFormat.format(error,
					"sb", org.sbml.libsbml.SBase.class.getName()));
		}
	...
	/* (non-Javadoc)
	 * @see org.sbml.jlibsbml.SBMLWriter#saveParameterProperties(org.sbml.jlibsbml.Parameter, java.lang.Object)
	 */
	...
	/* (non-Javadoc)
	 * @see org.sbml.jlibsbml.SBMLWriter#saveReactionProperties(org.sbml.jlibsbml.Reaction, java.lang.Object)
	 */
	private void saveReactionProperties(Reaction r, Object reaction)
		...
		for (i = ro.getNumReactants() - 1; i >= 0; i--) {
			...
			boolean keep = false;
			for (int j = 0; j < r.getReactantCount() && !keep; j++) {
		...
		for (i = ro.getNumProducts() - 1; i >= 0; i--) {
			...
			boolean keep = false;
			for (int j = 0; j < r.getProductCount() && !keep; j++) {
		...
		for (ModifierSpeciesReference mr : r.getListOfModifiers()) {
			...
			if (contains < 0) {
				...
				saveModifierSpeciesReferenceProperties(mr, ro.getModifier(contains));
			}
		}
		...
		for (i = ro.getNumModifiers() - 1; i >= 0; i--) {
			...
			boolean keep = false;
			for (int j = 0; j < r.getModifierCount() && !keep; j++) {
	...
	/* (non-Javadoc)
	 * @see org.sbml.SBMLWriter#saveSBaseProperties(org.sbml.SBase, java.lang.Object)
	 */
	...
	/* (non-Javadoc)
	 * @see org.sbml.jlibsbml.SBMLWriter#saveSpeciesProperties(org.sbml.jlibsbml.Species, java.lang.Object)
	 */
	private void saveSpeciesProperties(Species s, Object species) {
		...
		if (s.isSetInitialAmount()) {
			...
			if (!spec.isSetInitialConcentration()
					|| (s.getInitialConcentration() != spec.getInitialConcentration())) {
	...
	/* (non-Javadoc)
	 * @see org.sbml.squeezer.io.AbstractSBMLWriter#saveSpeciesReferenceProperties(org.sbml.SpeciesReference, java.lang.Object)
	 */
	private void saveSpeciesReferenceProperties(SpeciesReference sr,
		...
		if (!(specRef instanceof org.sbml.libsbml.SpeciesReference)) {
			throw new IllegalArgumentException(MessageFormat.format(error,
					"specRef", org.sbml.libsbml.SpeciesReference.class.getName()));
		}
		...
		if (sr.isSetStoichiometryMath()) {
			if (sp.isSetStoichiometryMath()
					&& !equal(sr.getStoichiometryMath().getMath(), sp.getStoichiometryMath().getMath())) {
				saveMathContainerProperties(sr.getStoichiometryMath(), sp.getStoichiometryMath());
			} else {
				sp.setStoichiometryMath(writeStoichoimetryMath(sr.getStoichiometryMath()));
			}
		} else {
	...
	private org.sbml.libsbml.Compartment writeCompartment(
		...
		if (compartment.isSetCompartmentType()
				&& !c.getCompartmentType().equals(compartment.getCompartmentType())) {
	...
	/* (non-Javadoc)
	 * @see org.sbml.jlibsbml.SBMLWriter#writeKineticLaw(org.sbml.jlibsbml.KineticLaw)
	 */
	private org.sbml.libsbml.KineticLaw writeKineticLaw(KineticLaw kinteicLaw,
		...
		for (LocalParameter p : kinteicLaw.getListOfLocalParameters()) {
			if (p.isSetUnits()) {
				if (!Unit.isUnitKind(p.getUnits(), p.getLevel(), p.getVersion())) {
					if (modelOrig.getUnitDefinition(p.getUnits()) == null) {
						modelOrig.addUnitDefinition(writeUnitDefinition(p.getUnitsInstance()));
					}
				}
			}
	...
	/* (non-Javadoc)
	 * @seeorg.sbml.SBMLWriter#writeModifierSpeciesReference(org.sbml.ModifierSpeciesReference)
	 */
	...
	public boolean writeSBML(Object sbmlDocument, String filename,
		...
		if (!(sbmlDocument instanceof org.sbml.libsbml.SBMLDocument)
			...
			throw new IllegalArgumentException(MessageFormat.format(error,
					"sbmlDocument", org.sbml.libsbml.SBMLDocument.class.getName()));
		}
	...
	private org.sbml.libsbml.SpeciesReference writeSpeciesReference(
		...
		if (speciesReference.isSetStoichiometryMath()) {
			sr.setStoichiometryMath(writeStoichoimetryMath(
	...
	private org.sbml.libsbml.UnitDefinition writeUnitDefinition(
		...
		for (Unit u : unitDefinition.getListOfUnits()) {
			...
			case libsbmlConstants.LIBSBML_VERSION_MISMATCH:
			  ...
			  logger.error("unit L" + unit.getLevel() + "\tV" + unit.getVersion());
			  logger.error("udef L" + ud.getLevel() + "\tV" + ud.getVersion());
				break;
