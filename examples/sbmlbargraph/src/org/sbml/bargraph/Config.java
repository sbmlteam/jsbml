...
public class Config
{
    ...
    public static final int operatingSystem()
    {
        if (osID == -1)
        {
            ...
            if ("Windows NT".equals(osName))         osID = OS_WINNT;
            else if ("Windows 95".equals(osName))    osID = OS_WIN95;
            else if ("Windows 98".equals(osName))    osID = OS_WIN98;
            else if ("Windows 2000".equals(osName))  osID = OS_WIN2000;
            else if ("Windows XP".equals(osName))    osID = OS_WINXP;
            else if ("Windows Vista".equals(osName)) osID = OS_WINVISTA;
            else if ("Windows 7".equals(osName))     osID = OS_WIN7;
            else if ("Windows 8".equals(osName))     osID = OS_WIN8;
            else if ("Solaris".equals(osName))       osID = OS_SOLARIS;
            else if (osName.startsWith("SunOS"))     osID = OS_SOLARIS;
            else if ("Linux".equals(osName))         osID = OS_LINUX;
            else if ("FreeBSD".equals(osName))       osID = OS_FREEBSD;
            else if ("HP-UX".equals(osName))         osID = OS_HP;
            else if ("AIX".equals(osName))           osID = OS_AIX;
            else if ("Irix".equals(osName))          osID = OS_IRIX;
            else if ("SunOS".equals(osName))         osID = OS_SUNOS;
            else if ("Digital UNIX".equals(osName))  osID = OS_DEC;
            else if ("OS/2".equals(osName))          osID = OS_OS2;
            else if (osName.startsWith("Mac OS"))    osID = OS_MAC;
            else if (osName.startsWith("Darwin"))    osID = OS_DARWIN | OS_MAC;
            else
                osID = OS_OTHER;
        }
        return osID;
