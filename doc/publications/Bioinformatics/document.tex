...
easily released as Java\texttrademark{} Web Start applications. JSBML?s internal data structures
have been developed entirely de novo based on the de?nitions in the SBML speci?cations. However,
it strives to achieve the highest possible degree of compatibility to the existing library
libSBML. JSBML supports all SBML levels and versions that are available today.
In addition, JSBML provides modules that facilitate the development of plugins of applications
such as CellDesigner or ease the migration from a libSBML backend.
...
a portable, robust, and easy-to-use library.
...
obsolete) are marked as deprecated. For elements sharing common properties,
JSBML defines a superclass or an interface for them. For instance, the interface
...
\begin{figure*}
\centerline{
  \subfloat[The SBML parser in JSBML considers the hierarchical data structure
...
such as visualizing it in a \texttt{JTree}, \texttt{Serializable} for saving
JSBML  objects in binary form or sending them via a network connection, and
\texttt{Cloneable} to create deep object copies.}
\label{fig:JSBML}
\end{figure*}
