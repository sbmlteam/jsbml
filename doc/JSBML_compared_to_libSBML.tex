...
important to keep in mind that SBML is a language that defines how to store and
to exchange models of biological processes between
...
\begin{figure}[p]
 ...
 adapted from \citep{Draeger2011}. JSBML refers to those components of a model
...
and hence be easily visualized. Listing~\vref{lst:Visualization} demonstrates in
a simple code example how to parse an SBML file
\index{SBML!XML file}%
...
The \texttt{ASTNode} class in JSBML\index{ASTNode@\texttt{ASTNode}} also
...
an expected Level/Version combination of an element more easily, as the
following example demonstrates.
\begin{lstlisting}[language=Java,float=h,captionpos=t,
title={Check for a minimal expected Level/Version combination:}]
...
whose classes and methods are structured and named and behave like classes and
...
\end{lstlisting}
With these constructors, dedicated single nodes can be created whose type will
be \texttt{ASTNode.Type.NAME}
\index{ASTNode@\texttt{ASTNode}!ASTNode.Type@\texttt{ASTNode.Type}}%
...
represented with a \texttt{short} value in Java to a value in $\mathbb{R}$, i.e.,
a \texttt{double} value. For this reason, the method
...
return some error codes instead.
...
\index{exception}%
on the other hand this will prevent you from doing obvious mistakes. One
potential problem of detecting errors quickly and throwing exceptions is that it
might prevent JSBML to be able to read properly invalid SBML models.
...
\index{ListOf*@\texttt{ListOf*}!\texttt{Filter}}%
There you can also find an \texttt{OrFilter} and an \texttt{AndFilter}, which take as arguments multiple other
...
Level~1\index{SBML!Level~1} for \texttt{ExplicitRule} and before SBML in Level~2,
...
\texttt{SBaseChangedListener}\index{event!\texttt{SBaseChangedListener}}\index{event!\texttt{SBaseChangedListener}} extends the interface \texttt{EventListener}
...
to quickly identify elements, based on their identifier or name. Developers can
search for different instances of \texttt{SBase} (for instance,
\texttt{CallableSBase},
...
% see documentation there : http://logging.apache.org/log4j/1.2/manual.html

% how to modify the default behavior
%Use -Dlog4j.configuration=/home/user/myLog4j.properties

% some examples configurations

\begin{lstlisting}[language=bash,caption={A simple  log4j example.},label=lst:log4j1]
...
\subsubsection{libSBMLio or how to use libSBML for parsing SBML into JSBML data structures?}
...
\subsubsection{cellDesigner or how to turn a JSBML-based application into a CellDesigner plugin?}
...
\subsubsection{libSBMLcompat, the JSBML's compatibility module for libSBML}

The compatibility module of JSBML will use the same package structure as the
libSBML java bindings and provides identically named classes and API. Using the module, it will be possible to
swich an existing application from libSBML to JSBML or the other way arround
