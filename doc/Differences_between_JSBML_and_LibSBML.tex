...
properties. In this way, the type hierarchy of JSBML\index{Application
programming interface!JSBML} has become more complex (see
...
\end{figure}
Just as in libSBML\index{Application programming interface!libSBML} \citep{Bornstein2008}, all elements extend the abstract type \texttt{SBase}\index{\texttt{SBase}}, but in
...
without explicitly writing them to an SBML file\index{SBML!XML file}. In this way, programs can easily
...
JSBML's\index{\texttt{SBase}} \texttt{SBase} interface can be directly passed to
...
\index{\texttt{SBase}!\texttt{AbstractSBase}}
...
\index{\texttt{SBase}!\texttt{NamedSBase}}
\index{\texttt{SBase}!\texttt{AbstractNamedSBase}}
...
\index{\texttt{SBase}!\texttt{SBaseWithDerivedUnit}}
...
\index{\texttt{SBase}!\texttt{NamedSBaseWithDerivedUnit}}
...
\index{\texttt{SBase}!\texttt{AbstractNamedSBaseWithUnit}}
\index{\texttt{SBase}!\texttt{AbstractNamedSBase}}
...
\index{JSBML!\texttt{QuantityWithUnit}}
...
\texttt{ASTNode}\index{\texttt{ASTNode}}).
The abstract class \texttt{AbstractMathContainer}\index{JSBML!\texttt{MathContainer}} serves as actual superclass
for most of the derived types. Figs.~\vrefrange{fig:MathContainer}{fig:MathContainerHierarchy} give a better overview of how this data structure
is intended to function.


\subsection{The \texttt{Assignment} interface}

JSBML\index{JSBML!\texttt{Assignment}} unifies all those elements that may
change the value of some \emph{variable} in SBML\index{SBML}
...
\texttt{MathContainer}\index{JSBML!\texttt{MathContainer}}). Therefore,
...
class to make sure that switching from libSBML\index{\texttt{InitialAssignment}}
to JSBML is quite smoothly.
However, the preferred way in JSBML\index{JSBML!\texttt{Variable}} is to apply
the methods \texttt{setVariable} either with \texttt{String}
\index{\texttt{String}!Identifier} or \texttt{Variable} instances as arguments.
...
the differences in the programming languages \texttt{C++}\index{\texttt{C++}}
...
language-like \texttt{String}s\index{\texttt{String}!Formula}, or from a MathML\index{MathML} representation. The JSBML\index{\texttt{ASTNode}}
...
\index{\texttt{ASTNode}!\texttt{ASTNodeValue}}
\index{\texttt{ASTNode}!\texttt{ASTNodeCompiler}} object,
which wraps the possible evaluation results of the interpretation.
JSBML\index{\texttt{ASTNode}} already provides several implementations of this
interface, for instance, \texttt{ASTNode} objects can be directly translated to
\texttt{C} language-like \texttt{String}s\index{\texttt{String}!Formula},
\LaTeX{}\index{\LaTeX}, or MathML\index{MathML} for further processing.
Furthermore, the class \texttt{UnitsCompiler}
\index{Unit!\texttt{\texttt{UnitsCompiler}}}, which JSBML uses to derive the unit of an abstract syntax tree, also implements this interface.
...
\begin{itemize}
 ...
 \texttt{Parameter}\index{\texttt{Parameter}}
\index{\texttt{Parameter}} whose constant attribute has been set to
...
all instances of \texttt{SBase}\index{\texttt{SBase}} in JSBML
...
\texttt{enum} \texttt{Type}\index{\texttt{ASTNode}!\texttt{ASTNode.Type}}.
...
\texttt{JSBML}\index{\texttt{libSBML}}\index{\texttt{libSBML}}. You can
therefore only find a class \texttt{JSBML}\index{JSBML!\texttt{JSBML}}. This
...
a \texttt{String}\index{\texttt{String}!Unit}\index{Unit!\texttt{String}} and
a corresponding \texttt{Unit.Kind}\index{Unit!\texttt{Unit.Kind}} can be done
by using the method
\begin{lstlisting}
...
\texttt{C}-like
formula \texttt{String}s\index{\texttt{String}!Formula} according to the specification of SBML Level 1\index{SBML!Level~1}
...
\index{Application programming interface!libSBML!Units!\texttt{getExponent()}},
...
1 and 2\index{SBML!Level~1} \citep{Hucka2003} \texttt{Event}s could be
explicitly equipped with units. The same holds true for instances of
\texttt{ExplicitRule}\index{Rule!\texttt{ExplicitRule}}
and \texttt{KineticLaw},\index{\texttt{KineticLaw}}
which both can only explicitly be populated with units for
SBML in Level 1, Versions 1 and 2\index{SBML!Level~1}. In contrast, \texttt{QuantityWithUnit}\index{JSBML!\texttt{QuantityWithUnit}} serves
as the abstract superclass for \texttt{LocalParameter}\index{\texttt{LocalParameter}} and \texttt{Symbol}\index{JSBML!\texttt{Symbol}},
...
\begin{description}
 \item[\texttt{getUnit()}] This method returns the \texttt{String}\index{\texttt{String}!Unit}\index{Unit!\texttt{String}} of the unit
 ...
 returns an empty \texttt{String}\index{\texttt{String}!Empty}.
\end{description}
...
\index{JSBML!\texttt{OverdeterminationValidator}} in JSBML provides methods to
...
from the libSBML Java API\index{Application programming interface!libSBML} and some new tests.
...
\item The \texttt{getTypeCode()}\index{\texttt{SBase}!\texttt{getTypeCode()}} methods are missing because due to the
\texttt{getClass()} methods in each element these are actually not neede when
working with Java.
\item The \texttt{toSBML()}\index{\texttt{SBase}!\texttt{toSBML()}} methods in \texttt{SBase} are still missing.
\item Constructors and methods with namespaces are not yet provided.
\item The libSBML compatibility module\index{libSBML!Compatibility module} could be improved.
\end{itemize}
...
\index{JSBML!As communication layer} between your application and libSBML
\citep{Bornstein2008} or the program CellDesigner \citep{Funahashi2003}.
Furthermore, a compatibility module\index{libSBML!Compatibility module} provides the same package structure as it is
...
\index{JSBML!\texttt{LibSBMLReader}} and
\texttt{LibSBMLWriter}\index{JSBML!\texttt{LibSBMLWriter}}.
...
\texttt{LD\_LIBRARY\_PATH}\index{libSBML!\texttt{LD\_LIBRARY\_PATH}} under Linux
operating system\index{Operating system}, appropriately. For details, see the
...
\textcolor{red}{To be described here.}\index{libSBML!Compatibility module}
