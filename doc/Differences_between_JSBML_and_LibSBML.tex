...
The intension of implementing a pure Java\texttrademark{}
...
From the very beginning JSBML has
been designed based on the SBML specifications \citep{Hucka2003, Hucka2008,
...
also the libSBML has grown
historically. The implementation of JSBML permitted to entirely
re-design the type hierarchy and the way of how to implement what is specified
in the SBML documents. However, it is important to keep in mind that SBML is a
language that defines how to store and to exchange (biochemical) models between
\index{Model!Storage and exchange}
different software tools. It does not specify how to represent its elements in
memory. Furthermore, during the evolution of SBML some elements or properties of
elements have become obsolete.
\index{Deprecation}
...
that libSBML bases on \texttt{C} and \texttt{C++} code
\index{C@\texttt{C}}
\index{C++@\texttt{C++}}
...
modules JSBML can also be used as a communication layer between an application
and other applications, such as CellDesigner \citep{Funahashi2003}. The
following sections will not only give an overview about the most important
differences between JSBML and libSBML, but also provide some programming
...
\end{figure}
Just as in libSBML\index{Application programming interface!libSBML} \citep{Bornstein2008}, all elements extend the abstract type \texttt{SBase},
...
The SBML\index{SBML} specifications define the data type \texttt{SBase}
\index{SBase@\texttt{SBase}}
\index{SBML!Specification}
as the abstract
...
the \texttt{clone()} method manually. Since all instances of \texttt{SBase}\index{SBase@\texttt{SBase}} and also
...
the latest specification of SBML\index{SBML}.
\index{SBML!Specification}
\index{Deprecation}
Hence, JSBML provides methods and
...
Generally, JSBML\index{Exception} throws more exceptions than libSBML,\index{Exception!Error codes} whose methods often return error codes instead.
This behavior helps
...
not necessarily have an identifier. Only instances of \texttt{NamedSBase}
\index{SBase@\texttt{SBase}!\texttt{NamedSBase}}
may have the fields identifier and name set. Hence, generally, the
\texttt{ListOf} class cannot assume these fields to be present. To query an
...
interface \texttt{SBaseWithUnit}.
\index{SBase@\texttt{SBase}!\texttt{SBaseWithUnit}}
This interface provides methods for direct
...
SBML in Level 1, Versions 1 and 2\index{SBML!Level~1}. In contrast,
...
\item The number of tests files is currently limited; needed are both tests
from the libSBML Java\texttrademark{} API\index{Application programming interface!libSBML} and some new tests.
...
\item The \texttt{getTypeCode()}
\index{SBase@\texttt{SBase}!\texttt{getTypeCode()}}
methods are missing because due to the \texttt{getClass()} methods in each
element, these are actually not needed when working with Java.
...
Furthermore, a compatibility module
\index{libSBML!Compatibility module}
provides the same package structure as it is provided by libSBML. In this
section, we will give small code examples of how to make use of these modules.
