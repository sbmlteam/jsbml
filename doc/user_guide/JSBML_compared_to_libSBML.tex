...
in \vref{fig:LevelVersionCheck} demonstrates. Method
...
if (mySBase.getLevelAndVersion().compareTo(Integer.valueOf(2), Integer.valueOf(2)) < 0) {
  throw new IllegalArgumentException(MessageFormat.format(
          "Cannot create a {0} with Level = {1,number,integer} and Version = {1,number,integer}.",
...
\item The list of \TreeNodeChangeListener objects is needed in all other
  ...
  each time receive the identical value as it was before. Since it is totally
  unclear of which type listeners are, a deep cloning of these is not
  possible.
...
\item[\code{nodeRemoved(TreeNode node)}:] This method notifies the listener
  that the given \TreeNode instance has just been removed, and therefore is
  no longer be a part of the \SBMLDocument.  (For details, see
  \vref{sec:howToExtension}.) The entire \SBMLDocument will not contain any
  pointers to this node anymore; however, the node itself will still contain
  a pointer to its former parent. (This makes it possible to recognize where
...
limited circumstances, beause each call invokes the matching algorithm---an
...
code example illustring the use of \LibSBMLReader.  The program displays the
