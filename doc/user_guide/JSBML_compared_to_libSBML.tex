...
\begin{sidewaysfigure}[p]
  ...
  \caption[The type hierarchy of the main SBML constructs in JSBML]{The
    ...
    hierarchies of objects are needed. In order to support SBML extension
    packages, the new interface \SBasePlugin and its abstract implementation
    \AbstractSBasePlugin have been added to JSBML in version 1.0 (both types
...
group, such as all \UnitDefinition instances, must be unique if these are
defined. The Boolean method \code{isIdMandatory()} in \NamedSBase indicates
...
implements \UniqueNamedSBase the identifier might be optional, as this is the
...
The only two elements with non-unique identifiers are \UnitDefinition, whose
...
If you work with SBML Level~1, where the specifications only define names, but
no identifiers for the elements, and call \code{setName(String)} the call will
be redirected to \code{setId(String)}. For all other Levels the \emph{name} can
be specified in addition to the \emph{id} and does not have to be unique at
all. In contrast to the identifier, there is no syntax check on the name,
because it can consist of any UTF-8 character. The class where to look for code
that manipulates identifiers and names is \AbstractNamedSBase. All JSBML classes
that have identifiers and names (either optionally or mandatory) are derived
from \AbstractNamedSBase. Accordingly, the \code{getName()} method also returns
the identifier if working with SBML Level~1. In this case, the call of the
\code{getName()} method is redirected to \code{getId()}. For all other Levels,
