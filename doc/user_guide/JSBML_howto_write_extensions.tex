...
about the extension, preferrably with a subdirectory of its own, \code{img},
...
\code{ExampleConstants} and an enum \code{ExampleListType} are used.
...
\code{ListOfFoos} is not a child of the original model, but of the example
...
The \code{processEndElement()} method is called whenever a closing tag is
